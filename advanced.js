...
// var arr1: Array<Cars> = [];
class Cars {
    constructor(group, name, color, consumption, image) {
        this.group = group;
        this.name = name;
        this.color = color;
...
document.getElementById("row").innerHTML += BMW.Infos();
document.getElementById("row").innerHTML += MERCEDES.Infos();
document.getElementById("row").innerHTML += AUDI.Infos();
let cycles = [];
class Motorcycles extends Cars {
    constructor(group, name, color, consumption, image, type) {
...
for (let i in cycles) {
    document.getElementById("cycles").innerHTML += cycles[i].Infos();
}
